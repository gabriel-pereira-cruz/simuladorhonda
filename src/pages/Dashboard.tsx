...
import { pathForSimulacaoTipo, subscribeSimulacoesChanged } from "@/lib/simulationHistory";
import { toast } from "sonner";
import { StatusBadge, ProgressBar } from "@/components/StatusBadge";
import { ScenarioSummary, SectionHeader } from "@/components/ScenarioSummary";
import { fetchDashboardData, type DashboardPayload } from "@/lib/mockApi";
import {
  CalendarRange,
  CalendarDays,
  Clock,
...
export default function Dashboard() {
  ...
  const [state, setState] = useState<
    { status: "loading" } | { status: "error"; message: string } | { status: "success"; data: DashboardPayload }
  >({ status: "loading" });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <SectionHeader
            ...
            title="Racks que mais restringem o plano (projeção)"
            right={
          ...
          <Card className="rounded-2xl border border-border/80 bg-card p-5 shadow-sm">
            <div className="divide-y divide-border/50">
              {data.racksDestaque.map((r) => (
                <div
                  key={r.codigo}
                  className="grid grid-cols-12 items-center gap-3 py-3.5 transition-colors duration-150 first:pt-0 last:pb-0 hover:bg-muted/35"
                >
